...
void entra(Monitor * m)
{
    ...
    {
        m->n_in_coda1++;
        pthread_cond_wait(&m->coda1, &m->lock);
        m->n_in_coda1--;
    }
...
void entra2(Monitor *m, int * scelta)
{
    ...
    {
        m->n_in_coda2++;
        pthread_cond_wait(&m->coda2, &m->lock);
        m->n_in_coda2--;
    }
...
void init_monitor(Monitor * monitor)
{
    ...
    pthread_cond_init(&monitor->coda1, NULL);
    pthread_cond_init(&monitor->coda2, NULL);
    
    monitor->n_in_coda1 = 0;
    monitor->n_in_coda2 = 0;
}
